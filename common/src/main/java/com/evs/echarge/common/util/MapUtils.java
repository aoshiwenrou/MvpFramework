...
public class MapUtils {
     ...
     * 跳转百度地图
     */
    public static void goToBaiduMap(Context context, double lng, double lat, String name) {
        if (!isInstalled("com.baidu.BaiduMap")) {
            ToastUtils.showShort("请先安装百度地图客户端");
            return;
        }

        // 一直用的高德体系，所以这里需要换成百度的
        double[] baidu = gcj02ToBd09(lng, lat);
        Intent intent = new Intent();
        intent.setData(Uri.parse("baidumap://map/direction?destination=latlng:" + baidu[1] + "," + baidu[0]
                + "|name:" + name + // 终点
                "&mode=driving" + // 导航路线方式
     ...
     * 跳转高德地图
     */
    public static void goToGaodeMap(Context context, double lng, double lat, String name) {
        ...
        stringBuffer.append("&lat=").append(lat)
                .append("&lon=").append(lng).append("&keywords=" + name)
                .append("&dev=").append(1)
